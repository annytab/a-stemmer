...
namespace Annytab
{
    ...
    public class SwedishStemmer : Stemmer
    {
        ...
        /// <summary>
        /// Get the swedish steam word from a specific word
        /// </summary>
        ...
        public string GetSteamWord(string word)
        {
            ...
            // Get the index of the first non-vowel after the first vowel
            int firstNonVowel = GetFirstNonVowel(characters);
            ...
            for (int i = 0; i < this.endingsStep1.Length; i++)
            {
                ...
                if (part2.ToLower().EndsWith(this.endingsStep1[i]))
                {
            ...
            // Make sure that we compare on lowercase letters
            if (part2.ToLower().EndsWith("s"))
            {
                // Create a full string of part1 and part2
                string fullWord = part1 + part2;

                // Make sure that we compare on lowercase letters
                char[] fullWordChars = fullWord.ToLower().ToCharArray();

                // Get the preceding char before the s
                char precedingChar = fullWordChars[fullWordChars.Length - 2];
            ...
            {
                // Make sure that we compare on lowercase letters
                if (part2.ToLower().EndsWith(this.endingsStep2[i]))
                {
                    // Delete the ending
                    part2 = part2.Remove(part2.Length - 1);
                    break;
            ...
            // **********************************************
            // Make sure that we compare on lowercase letters
            if (part2.ToLower().EndsWith("fullt") || part2.ToLower().EndsWith("löst"))
            {
            ...
            else
            {
                ...
                {
                    // Make sure that we compare on lowercase letters
                    if (part2.ToLower().EndsWith(this.endingsStep3[i]))
                    {
                        // Delete the ending
        ...
        } // End of the GetSwedishStem method
        ...
        /// <summary>
        /// Get the first index of a non-vowel following a vowel
        /// </summary>
        /// <param name="charArray">An array of chars</param>
        /// <returns>An index as an int</returns>
        private int GetFirstNonVowel(char[] charArray)
        {
            // A boolean that indicates if a char is vowel
            bool isVowel = false;
            ...
            for (int i = 0; i < charArray.Length; i++)
            {
                ...
                {
                    // Check if we have found the first non-vowel after the first vowel
                    if (isVowel == true && charArray[i] != this.vowels[j])
                    {
                        // There must be at least three characters before the first vowel
                        if (i < 3)
                        {
                            return 3;
                        }
        ...
        } // End of the GetFirstNonVowel method
